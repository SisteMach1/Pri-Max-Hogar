...
let carrito=[]; let filtroActual='todos';
function renderProductos(){
  ...
  `).join('') || '<p style="grid-column:1/-1; text-align:center; padding:40px; background:#fff; border-radius:12px">No hay productos. Agregá desde el panel Admin.</p>';
}
...
function actualizarCarrito(){
  const countEl=document.getElementById('cart-count'); if(countEl) countEl.innerText=carrito.length;
  ...
  `).join('') || '<p style="padding:20px; text-align:center; color:#999">Tu carrito está vacío<br><small>Agregá productos</small></p>';
  ...
  const nota=document.getElementById('cli-nota')?.value||'';
  let mensaje=`Hola Pri-Max Hogar!%0A%0A*PEDIDO NUEVO*%0A`;
  carrito.forEach(p=> mensaje+=`- ${p.nombre} $${p.precio.toLocaleString('es-AR')}%0A`);
  mensaje+=`%0A*Total: $${total.toLocaleString('es-AR')}*%0A%0A*DATOS:*%0ANombre: ${nombre}%0ADir: ${direccion}%0ATel: ${telefono}%0APago: ${pago}%0A`;
